...
def _normalize_ft(ft_obs: np.ndarray, ft_stats: dict) -> np.ndarray:
    """Normalize a 6-dim F/T reading to [-1, 1] using global dataset min/max."""
    ft_min = np.array(ft_stats["ft_min"], dtype=np.float32)
    ft_max = np.array(ft_stats["ft_max"], dtype=np.float32)
    denom = np.where((ft_max - ft_min) < 1e-8, 1.0, ft_max - ft_min)
    return (2.0 * (ft_obs.astype(np.float32) - ft_min) / denom - 1.0).clip(-1.0, 1.0)
...
def get_action(
    ...
    worker_id: int = 0,
    batch_size: int = 1,
    ft_obs: Optional[np.ndarray] = None,
    ...
    with torch.inference_mode():
        ...
        # Add blank placeholder for current F/T (F/T will be injected into latent later)
        use_ft = getattr(cfg, "use_ft", False)
        ...
        # Add blank placeholder for future F/T (generated by model at inference; -1 means not injected)
        ...
        data_batch = {
            ...
            # F/T latent injection (only used when model was trained with use_ft=True)
            "current_ft_latent_idx": torch.tensor([current_ft_latent_idx] * batch_size, dtype=torch.int64).cuda(),
            "future_ft_latent_idx": torch.tensor([-1] * batch_size, dtype=torch.int64).cuda(),
            "current_ft": (
                torch.from_numpy(
                    _normalize_ft(ft_obs, ft_stats)
                ).reshape(batch_size, -1).to(dtype=torch.bfloat16).cuda()
                if use_ft and ft_obs is not None and ft_stats is not None
                else None
            ),
            "future_ft": None,
        }
